...
class BluetoothSource @Inject constructor(
    ...
    suspend fun runBluetooth(source: String): Flow<BluetoothResponse> = callbackFlow {
        launch(Default) {
            if (checkSelfPermission()) {
                runCatching { adapter.enable() }.onSuccess {
                    runCatching { socket?.close() }.onSuccess {
                        runCatching { delay(1000L); socket = create }.onSuccess {
                            runCatching { work(source) { trySend(it) } }
                                .onFailure { stop }.error(this@callbackFlow)
                        }.error(this@callbackFlow)
                    }.error(this@callbackFlow)
                }
            }
        }
                ...
                while (isActive) {
                    reader?.readLine()?.apply {
                        if (isNotEmpty() && this != "nAn") {
                            when (counter) {
                                0 -> {
                                    if (writeTemp) { writeTemp(this); writeTemp = false }
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    private fun <T> Result<T>.error(scope: ProducerScope<BluetoothResponse>) =
        onFailure { e -> scope.trySend(e.error) }

    @OptIn(ExperimentalCoroutinesApi::class)
    private val ProducerScope<BluetoothResponse>.success get() = trySendBlocking(BluetoothResponse.OnSuccess)
